...
function init(){
	...
	if(type=="flv"){
		flv_server_url = loadFlvServer();
		if(flv_server_url==""){
			alert("无可用FLV流媒体服务器，请联系系统管理员配置！");
		}
	}else{
		rtmp_server_url = loadRtmpServer("1078-playback");
		if(rtmp_server_url==""){
			alert("无可用RMTP流媒体服务器，请联系系统管理员配置！");
		}
	}
...
function openVideoDialog(dialogId) {
	playTimeHandler(0);
	...
	ajaxAsyncPost(url, data, function(result) {
		if (result.code != 0) {
			...
			if(type=="flv"){
				...
				current_video_info.src = flv_server_url.replace("{simCode}",simCode).replace("{channel}",current_video_info.channelId);
				current_video_info.player = initFlvPlayer(current_video_info);
				openVideoDialog("history-flv-player-dlg");
			}else{
				if(rtmp_server_url==""){
					alert("无可用RTMP流媒体服务器，请联系系统管理员配置！");
					return "";
				}
				
				if(!hasUsableFlash()){
					showMessage("您的浏览器不支持FlashPlayer插件或没有启用该插件");
					$("#history-rtmp-player").html("<div style='width: 100%; height: 100%; color: rgb(255, 255, 255); text-align: center; padding-top: 40px;'><p>您的浏览器不支持FlashPlayer插件或没有启用该插件</p><p><a href='https://www.flash.cn/' target='_blank' style='color:#FFFFFF'>点击下载FlashPlayer插件</a></p></div>");
				}else{
					current_video_info.src = rtmp_server_url.replace("{simCode}",simCode).replace("{channel}",current_video_info.channelId);
					if(type=="ck"){
						current_video_info.player = initCkplayer(current_video_info);
					}else{
						current_video_info.player = initRtmpPlayer(current_video_info);
					}
				}
				openVideoDialog("history-rtmp-player-dlg");
			}
		}
	},true);
}
...
function stopPlayBack(){
	if(current_video_info){
		...
		if(type=="flv"){
			try{
				current_video_info.player.unload();
			...
			if(type=="ck"){
				...
				current_video_info.player.remove();
			}
		}
...
function initRtmpPlayer(videoInfo) {
	var player=  startRtmpLiveStream("history-rtmp-player",videoInfo.src,function(){
		stopPlayBack();
	});
	
	player.onAlive(function(){ 
		playTimeHandler(new Date().getTime());
	});
...
function initCkplayer(videoInfo) {
	var videoObject = {
			container: '#history-rtmp-player',//“#”代表容器的ID，“.”或“”代表容器的class
