...
function onUncheckBootstrapTableRow(row){
	for(var i=0;i<current_selected_device.length;i++){
		if(current_selected_device[i]==row[1]){
			current_selected_device[i] = "";
			$("#map-frame"+(i+1))[0].contentWindow.location.reload(true);
		}
	}
}
...
function onSelectDevice(deviceId){
	...
	if(index!=-1){
		current_selected_device[index]=deviceId;
	}else{
		...
		index = current_selected_device_index;
	}
	
	var frame = $("#map-frame"+index)[0].contentWindow;
