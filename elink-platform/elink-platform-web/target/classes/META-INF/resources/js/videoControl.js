...
var video_box_width,video_box_height;
...
function showVideoView(n) {
	...
	for (var i = 0; i < n; i++) {
		...
		if (player.length > 0) {
			player.height(widthHeight.height).width(widthHeight.width);
		} else {
			var html = '<div id="player-container'
					+ i
					+ '" class="video_box" style="width:'
					+ widthHeight.width
					+ 'px;height:'
					+ widthHeight.height
					+ 'px;"></div>';
...
function showDeviceInfoWindow(deviceId, lacation, gpsInfo) {
	...
	if(lacation){
		mapUtil.regeocoder(lacation, function(address) {
			...
			text += "<br/>1设&nbsp;&nbsp;备&nbsp;&nbsp;ID：" + device.simCode;
			text += "<br/>2数据时间：" + new Date(gpsInfo.gpsTime).format();
...
function startRtmpCkplayer(videoInfo) {
	var index = 0;
