...
public class VincentyGeodesy {
    ...
    public static LngLat moveInDirection(LngLat point, double bearingInDegrees, double distanceInMeters) {
        ...
        double tanU1 = (1 - f) * Math.tan(point.getLat() * degToRad);
        ...
        double newLon = point.getLng() + L / degToRad;

        newLon = (newLon > 180.0 ? 360.0 - newLon : newLon);
        newLon = (newLon < -180.0 ? 360.0 + newLon : newLon);

        return new LngLat(newLon, newLat);
    }
     ...
     * @return
     */
    public static double distanceInMeters(LngLat start, LngLat end) {
        double a = 6378137, b = 6356752.3142, f = 1 / 298.257223563; // WGS-84
        // ellipsiod
        double L = (end.getLng() - start.getLng()) * degToRad;
        double U1 = Math.atan((1 - f) * Math.tan(start.getLat() * degToRad));
        double U2 = Math.atan((1 - f) * Math.tan(end.getLat() * degToRad));
