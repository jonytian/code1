...
public class DeviceStateMessageHandler extends MessageHandler {
	...
	public void handle(ExchangeMessage message) throws Exception {
		if (message.getMessageId().equals(ExchangeMessage.MESSAGEID_GATEWAY_LINK_STATUS_MESSAGE)) {
			...
			if (device == null) {
				logger.warn("******无该设备信息，simCode={}", simCode);
				return;
			}
			...
			ValueWrapper cache = cacheManager
					.getCache(com.legaoyi.common.util.Constants.CACHE_NAME_DEVICE_STATE_CACHE.concat(gatewayId))
					.get(deviceId);
			...
			if (isNeedCache) {
				cacheManager.getCache(com.legaoyi.common.util.Constants.CACHE_NAME_DEVICE_STATE_CACHE.concat(gatewayId))
						.put(deviceId, stateMap);
				cacheManager.getCache(com.legaoyi.common.util.Constants.CACHE_NAME_DEVICE_GATEWAY_CACHE).put(deviceId,
						gatewayId);
				...
				if (state == 3) {// 上线检查上一次acc状态，如果是0，则为熄火
					ValueWrapper value = cacheManager
							.getCache(com.legaoyi.common.util.Constants.CACHE_NAME_DEVICE_GPS_CACHE).get(deviceId);
					if (value != null) {
				...
				cacheManager.getCache(com.legaoyi.common.util.Constants.CACHE_NAME_DEVICE_BIZ_STATE_CACHE).put(deviceId,
						bizState);

				ValueWrapper staticCache = cacheManager
						.getCache(com.legaoyi.common.util.Constants.CACHE_NAME_DEVICE_ONLINE_TIME_STATIC_CACHE)
						.get(deviceId);
				...
				if (state == 2) {
					...
					if (DateUtils.getDate(now) - DateUtils.getDate(new Date(lastTime)) > 0) {
						...
						total = (total + today - lastTime);
						long oneDay = 24 * 60 * 60 * 1000;
						yesterdayStat.put("total", total > oneDay ? oneDay : total);
				...
				cacheManager.getCache(com.legaoyi.common.util.Constants.CACHE_NAME_DEVICE_ONLINE_TIME_STATIC_CACHE)
						.put(deviceId, staticMap);
				...
				messageBatchSaveProducer
						.produce(new BatchMessage(Constants.BATCH_MESSAGE_TYPE_DEVICE_STATE, updateInfo));
