...
public class MessagePublishChannelHandler extends BaseMessageChannelInboundHandler {
    ...
    @Autowired
    @Qualifier("commonServerMessageHandler")
    private ServerMessageHandler commonMessageHandler;

    @Autowired
    @Qualifier("mediaServerMessageHandler")
    private ServerMessageHandler mediaMessageHandler;
    ...
    protected boolean handle(ChannelHandlerContext ctx, Message message) {
        ...
        try {
            ...
            if (urgentMessageMap.containsKey(messageId)) {
                urgentMessageHandler.handle(new ExchangeMessage(ExchangeMessage.MESSAGEID_GATEWAY_UP_MESSAGE, message.clone(), exchangeId));
            } else if (mediaMessageMap.containsKey(messageId)) {
                mediaMessageHandler.handle(new ExchangeMessage(ExchangeMessage.MESSAGEID_GATEWAY_UP_MEDIA_MESSAGE, message.clone(), exchangeId));
            } else {
                commonMessageHandler.handle(new ExchangeMessage(ExchangeMessage.MESSAGEID_GATEWAY_UP_MESSAGE, message.clone(), exchangeId));
            }
        } catch (Exception e) {
