...
public class ServerDisruptorMessageHandler implements DisruptorMessageHandler {
    ...
    private void resetMessage(ExchangeMessage exchangeMessage) throws Exception {
        ...
        JTT808_0801_MessageBody messageBody = (JTT808_0801_MessageBody) message.getMessageBody();
        ...
        // 多媒体文件直接存储磁盘
        String path = message.getMessageHeader().getSimCode().concat(File.separator).concat(DateUtils.format(new Date(), "yyyyMMdd")).concat(File.separator).concat(DateUtils.format(new Date(), "yyyyMMddHHmmss") + suffix);
        desc = desc.concat(path);
        FileUtil.writeWithMappedByteBuffer(desc, messageBody.getMediaData());
        // 音视频只发布存储路径，上层应用来处理
