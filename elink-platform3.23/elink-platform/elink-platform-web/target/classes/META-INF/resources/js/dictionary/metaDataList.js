...
function getQueryParams(){
	...
	if(type!=""){
		...
		params["type.gte"]= dictionary_entity;
		params["type.lte"]= dictionary_message_example;
	}
...
function getColumns(){
	return [{field: 'check',checkbox :true},
		{field:"type",title:"类型",formatter: function(value,row,index){
			 ...
			 if(type==1){
				 return "实体对象";
			 }else if(type==dictionary_entity_describe){
				 return "对象描述";
			 }else if(type==dictionary_message_describe){
				 return "消息描述";
			 }else if(type==dictionary_message_example){
				 return "消息样例";
			 }
		 }},
		 {field:"code",title:"代码"},
