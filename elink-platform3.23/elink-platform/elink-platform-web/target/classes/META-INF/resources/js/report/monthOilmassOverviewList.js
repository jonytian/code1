...
function getQueryUrl() {
	return report_api_server_servlet_path + "/query/gpsInfoDayReport.json?orderBy=totalOilmass&desc=true";
}
...
function getColumns() {
	...
	columns[0][1] = {
		...
		valign : 'middle',
		formatter : function(value, row, index) {
			return row.totalOilmass.toFixed(0);
		}
	};
	for (var i = 1; i <= lastDay; i++) {
		...
		columns[0][1 + i] = {
			...
			formatter : function(value, row, index) {
				if (value) {
					return value.oilmass.toFixed(0);
				}
				return "-";
			}
		};
...
function exportExcel() {
	...
	var url = report_api_server_servlet_path+ "/excel/monthOilmass/"+month+".json?deviceId="+deviceId;
