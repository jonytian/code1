var max_data_packet_Length = 475;
var seq=0,rec,startTalkTime,talkSimCode,talkChannel,talkDeviceId,audioClient;
...
function send1078VoiceMessage(simCode,channel,data,flag,time){
	...
	//包序号
	view.setUint16(offset, seq++, false);
	offset += 2;
	...
	for(var j=0;j<dataLength;j++){
		view.setInt16(offset, data[j], false);
		offset += 2;
		//view.setUint8(offset, data[j], false);
		//offset += 1;
	}
...
function recopen(){
	...
	rec=Recorder({
		onProcess:function(buffers,level,time,sampleRate){
			var interval = parseInt(time / (buffers.length * 2));
			$("#video_talker_voice").css("width",level+"%");
			
			var simCode=talkSimCode,channel=talkChannel;
			
			//这里封装1078协议
			var postTime = startTalkTime;
			var bytes=buffers;

			postTime += interval;
			var dataLength = bytes.length;
			if(dataLength > max_data_packet_Length){
				var startIndex = 0;
				while((startIndex + max_data_packet_Length) < dataLength){
					var endIndex = startIndex + max_data_packet_Length;
					var data = bytes.slice(startIndex,endIndex);
					send1078VoiceMessage(simCode,channel,data,(startIndex==0?1:2),postTime);
					startIndex=endIndex;
					postTime += interval;
				}
				var data = bytes.slice(startIndex);
				send1078VoiceMessage(simCode,channel,data,3,postTime);
			}else{
				send1078VoiceMessage(simCode,channel,bytes,0,postTime);
			}
		}
	});
