...
public class MessageServiceImpl implements MessageService {
    ...
    private void saveAlarmSetting(String enterpriseId, String deviceId, String messageId, Map<?, ?> map) throws Exception {
        ...
        if (StringUtils.isNotBlank(startTime)) {
            setting.setStartTime(DateUtils.parse(startTime.replace("0000-00-00", "1970-01-01"), DateUtils.DATE_FORMAT));
        }
        String endTime = (String) map.get("endTime");
        if (StringUtils.isNotBlank(endTime)) {
            setting.setEndTime(DateUtils.parse(endTime.replace("0000-00-00", "1970-01-01"), DateUtils.DATE_FORMAT));
        }
