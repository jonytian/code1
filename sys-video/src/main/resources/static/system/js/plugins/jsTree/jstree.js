 ...
 */
	...
	/**
	 * stores all defaults for the config
	...
	$.jstree.core.prototype = {
		...
		/**
		 * loads a node (fetches its children using the `config.data` setting). Multiple nodes can be passed to by using an array.
		...
		_load_node : function (obj, callback) {
			...
			if(!s) {
				...
				// return callback.call(this, obj.id === $.jstree.root ? this._append_html_data(obj, this._data.config.original_container_html.clone(true)) : false);
			}
			if($.isFunction(s)) {
		 ...
		 * @param {Number} animation the animation duration in milliseconds when opening the node (overrides the `config.animation` setting). Use `false` for no animation.
		 ...
		 * @param {Number} animation the animation duration in milliseconds when closing the node (overrides the `config.animation` setting). Use `false` for no animation.
		...
		get_state : function () {
			var state	= {
				'core' : {
					...
					'themes' : {
						'name' : this.get_theme(),
						'icons' : this._data.config.themes.icons,
						'dots' : this._data.config.themes.dots
					},
					*/
					'selected' : []
				}
		...
		check : function (chk, obj, par, pos, more) {
			...
			if(chc === false || ($.isFunction(chc) && chc.call(this, chk, obj, par, pos, more) === false) || (chc && chc[chk] === false)) {
				this._data.core.last_error = { 'error' : 'check', 'plugin' : 'core', 'id' : 'core_03', 'reason' : 'User config for config.check_callback prevents function: ' + chk, 'data' : JSON.stringify({ 'chk' : chk, 'pos' : pos, 'obj' : obj && obj.id ? obj.id : false, 'par' : par && par.id ? par.id : false }) };
				return false;
			}
			return true;
		},
		 ...
		 * @param {mixed} theme_url  the location of the CSS file for this theme. Omit or set to `false` if you manually included the file. Set to `true` to autoload from the `config.themes.dir` directory.
	...
	$.jstree.plugins.wholerow = function (options, parent) {
		this.bind = function () {
			...
			this.element
				...
				.on("init.jstree loading.jstree ready.jstree", $.proxy(function () {
						//div.style.height = this._data.config.li_height + 'px';
		...
		this.redraw_node = function(obj, deep, callback, force_render) {
			...
			if(obj) {
				...
				//tmp.style.height = this._data.config.li_height + 'px';
