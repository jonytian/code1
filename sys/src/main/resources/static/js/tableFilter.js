	...
	tableFilter.render = function(opt){
		...
		var main = function (){
			...
			layui.each(filters, function(i, filter){
				...
				var filterIcon = $('<span class="layui-table-filter layui-inline" ><i class="layui-icon '+icon+'" style="margin-left: 20px"></i></span>');

				if(th.find('.layui-table-cell').find('span').find('i').length == 0){
