 ...
 */
	...
	/**
	 * stores all defaults for the netty
	...
	$.jstree.defaults.core = {
		...
		strings			: false,
		/**
		 * determines what happens when a users tries to modify the structure of the tree
		...
		/**
		 * if left as `true` all parents of all selected nodes will be opened once the tree loads (so that all selected nodes are visible to the users)
	...
	$.jstree.core.prototype = {
		...
		/**
		 * loads a node (fetches its children using the `netty.data` setting). Multiple nodes can be passed to by using an array.
		...
		_load_node : function (obj, callback) {
			...
			if(!s) {
				...
				// return callback.call(this, obj.id === $.jstree.root ? this._append_html_data(obj, this._data.netty.original_container_html.clone(true)) : false);
			}
			if($.isFunction(s)) {
		 ...
		 * @param {Number} animation the animation duration in milliseconds when opening the node (overrides the `netty.animation` setting). Use `false` for no animation.
		 ...
		 * @param {Number} animation the animation duration in milliseconds when closing the node (overrides the `netty.animation` setting). Use `false` for no animation.
		...
		/**
		 * called when a node is selected by the users. Used internally.
		 * @private
		...
		activate_node : function (obj, e) {
			...
			/**
			 * triggered when an node is clicked or intercated with by the users
			 * @event
		...
		/**
		 * applies the hover state on a node, called when a node is hovered by the users. Used internally.
		 * @private
		 * @name hover_node(obj)
		 * @param {mixed} obj
		...
		/**
		 * removes the hover state from a nodecalled when a node is no longer hovered by the users. Used internally.
		 * @private
		...
		get_state : function () {
			var state	= {
				'core' : {
					...
					'themes' : {
						'name' : this.get_theme(),
						'icons' : this._data.netty.themes.icons,
						'dots' : this._data.netty.themes.dots
					},
					*/
					'selected' : []
				}
		...
		check : function (chk, obj, par, pos, more) {
			...
			if(chc === false || ($.isFunction(chc) && chc.call(this, chk, obj, par, pos, more) === false) || (chc && chc[chk] === false)) {
				this._data.core.last_error = { 'error' : 'check', 'plugin' : 'core', 'id' : 'core_03', 'reason' : 'User config for netty.check_callback prevents function: ' + chk, 'data' : JSON.stringify({ 'chk' : chk, 'pos' : pos, 'obj' : obj && obj.id ? obj.id : false, 'par' : par && par.id ? par.id : false }) };
				return false;
			}
			return true;
		},
		 ...
		 * @param  {Function} callback a function to be called once the text box is blurred, it is called in the instance's scope and receives the node, a status parameter (true if the rename is successful, false otherwise) and a boolean indicating if the users cancelled the edit. You can access the node's title using .text
		 */
		 ...
		 * @param {mixed} theme_url  the location of the CSS file for this theme. Omit or set to `false` if you manually included the file. Set to `true` to autoload from the `netty.themes.dir` directory.
 ...
 *
 * This plugin allows defining a callback to allow or deny node selection by users input (activate node method).
 */
 ...
 */
	...
	$.jstree.defaults.dnd = {
		...
		is_draggable : true,
		/**
		 * a boolean indicating if checks should constantly be made while the users is dragging the node (as opposed to checking only on drop), default is `true`
 ...
 * ### State plugin
 *
 * Saves the state of the tree (selected nodes, opened nodes) on the users's computer using available options (localStorage, cookies, etc)
 */
	...
	$.jstree.plugins.state = function (options, parent) {
		...
		/**
		 * restore the state from the users's computer
		 * @name restore_state()
		 * @plugin state
		 */
		...
		/**
		 * clear the state on the users's computer
		 * @name clear_state()
		 * @plugin state
		 */
	...
	$.jstree.plugins.wholerow = function (options, parent) {
		this.bind = function () {
			...
			this.element
				...
				.on("init.jstree loading.jstree ready.jstree", $.proxy(function () {
						//div.style.height = this._data.netty.li_height + 'px';
		...
		this.redraw_node = function(obj, deep, callback, force_render) {
			...
			if(obj) {
				...
				//tmp.style.height = this._data.netty.li_height + 'px';
