...
import com.example.logs.netty.util.OrderType;
import com.example.logs.netty.util.SendMessage;
import com.example.logs.netty.util.WebSocketConstant;
import com.example.logs.service.SendMsgService;
    ...
    public String uploadLog(@RequestBody List<LogVo> logVos){
        if(logVos.size()>0){
            logVos.stream().forEach(o->{
                ...
                data.put("download","1");
                data.put("exceptionName",o.getException());
                data.put("happenTime",o.getHappenTime());
                data.put("logPath",o.getLogPath());
                data.put("imei",o.getDevice().getImei());
                List<String> imeis = new ArrayList();
                imeis.add(o.getDevice().getImei());
                resultMap.put("data",data);
                SendMessage sendMessage = new SendMessage();
